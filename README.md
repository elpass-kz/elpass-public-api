...
```typescript
{
  ...
  photo: File;                   // Photo file (required for creation)
  zones?: string | string[];     // Access zones (optional) - e.g., "1", ["1", "2"], or "all"
  group?: string;                // Group: SITE_ID+APARTMENT_NO (optional)
  begin_at?: string;             // Validity start date ISO 8601 (optional, default: now)
  end_at?: string;               // Validity end date ISO 8601 (optional, default: now + 10 years)
  passType?: string;             // Pass type: "permanent" | "temporary" | "guest" | "blocked" (optional)
  meta?: object;           // Metadata for BigApp integration (optional)
...
| **photo** | File | ✅ Yes (POST) | Face photo file - required for creation, optional for updates |
| **zones** | String/Array | ❌ Optional | Access zones - can be "1", "all", or JSON array like ["1", "2"] |
...
| **end_at** | ISO 8601 DateTime | ❌ Optional | Card expiration date (default: current date + 10 years) |
| **passType** | String | ❌ Optional | Pass type: "permanent", "temporary", "guest", "blocked" |
| **meta** | JSON String | ❌ Optional | Metadata for BigApp integration (JSON stringified) |
...
> **Note**: For card creation, photo is **required**. For updates, photo is **optional** (only include if you want to change it).
...
Form fields:
  ...
  name: "Ivan Petrov"
  photo: <file upload>
  zones: "1"  (optional - can be "1", ["1", "2"], or "all")
  group: "SITE001+A101"  (optional)
  begin_at: "2025-01-15T00:00:00Z"  (optional)
  end_at: "2026-01-15T23:59:59Z"  (optional)
  passType: "permanent"  (optional)
  meta: "{\"complexName\":\"My Complex\",\"complexId\":\"123\"}"  (optional)
```
...
**A**: The `photo` field is **required** for card creation. You must provide a face photo during POST request. If you need to update the photo later, you can do so via PATCH update.
...
class ElpassCardManager:
    ...
    def create_card(self, resident_uuid, name, zones, photo_path, group=None, **kwargs):
    ...
    def update_card(self, card_uuid, photo_path=None, **fields):
        ...
        for key in ["name", "zones", "begin_at", "end_at", "passType", "group", "meta"]:
            if key in fields:
                data[key] = fields[key]
