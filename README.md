...
```typescript
{
  ...
  photo: File;                   // Photo file (required when no pin is provided)
  ...
  end_at?: string;               // Validity end date ISO 8601 (required when pin is provided, default: now + 10 years)
  passType?: string;             // Pass type: "permanent" | "guest" | "blocked" (optional)
  pin?: string;                  // PIN code for access (required for GUEST passType)
...
| **photo** | File | ⚠️ Conditional | Face photo file - **required when no pin is provided**, optional for updates |
...
| **end_at** | ISO 8601 DateTime | ⚠️ Conditional | Card expiration date - **required when pin is provided** (default: current date + 10 years) |
| **passType** | String | ❌ Optional | Pass type: "permanent", "temporary", "guest", "blocked" |
| **pin** | String | ⚠️ Conditional | PIN code for access - **required for GUEST passType** |
...
> **Note**: For card creation, photo is **required when no pin is provided**. If a `pin` is provided, photo becomes optional. For updates, photo is always **optional** (only include if you want to change it).
...
Form fields:
  ...
  name: "Ivan Petrov"
  photo: <file upload>  (required when no pin is provided)
  ...
  end_at: "2026-01-15T23:59:59Z"  (required when pin is provided, otherwise optional)
  passType: "permanent"  (optional, "guest" requires pin)
  pin: "1234"  (optional, required for GUEST passType)
...
### 2. Create Guest Card (with PIN, no photo)
...
> **Note**: For guest cards, `pin` and `end_at` are required, but `photo` is optional.
...
**A**: The `photo` field is **required when no pin is provided** for card creation. If you provide a `pin` code, the photo becomes optional. Note that when using a pin, the `end_at` field becomes required. If you need to update the photo later, you can do so via PATCH update.
...
class ElpassCardManager:
    ...
    def create_card(self, resident_uuid, name, object_guid, photo_path=None, group=None, entrance_number=None, pin=None, **kwargs):
        """Create new resident access card

        Note: photo_path is required when no pin is provided.
              end_at is required when pin is provided.
              pin is required for GUEST passType.
        """
        ...
        # Prepare file (required when no pin is provided)
        files = {}
        if photo_path:
    ...
    def update_card(self, card_uuid, photo_path=None, **fields):
        ...
        for key in ["name", "begin_at", "end_at", "passType", "group", "meta", "entranceNumber", "pin"]:
            if key in fields:
                data[key] = fields[key]
